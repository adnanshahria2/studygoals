...
import { EditTargetCardModal } from '@/components/modals/EditTargetCardModal';
...
type MobileTabType = 'dashboard' | 'targets' | 'daily' | 'settings';
...
export const Dashboard: React.FC = () => {
    ...
    const [editTargetCard, setEditTargetCard] = useState<TargetCardMeta | null>(null);
    const [deleteConfirm, setDeleteConfirm] = useState<{ tableId: string; cardId: string; targetCardId?: string } | null>(null);
    ...
    // Compute marks stats
    const { totalMarksAchieved, totalMarksPossible } = useMemo(() => {
    ...
    const validTargetCards = useMemo(() => {
        return (tableData.targetCards || []).filter((card): card is TargetCardMeta =>
    ...
    return (
        <div className="min-h-screen bg-bg-dark text-white font-sans selection:bg-accent-purple/30 pb-24 md:pb-0">
                            ...
                            <div className="flex-1 flex items-center gap-4 bg-bg-card/50 border border-accent-blue/30 shadow-[0_0_20px_-5px_rgba(59,130,246,0.25)] rounded-2xl p-3 hover:border-accent-blue/50 transition-colors">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-2xl font-bold text-white tabular-nums">{days}</span>
                        ...
                        <div className="space-y-4 animate-fadeIn">
                            ...
                            <div className="bg-gradient-to-br from-blue-900/30 via-slate-900/50 to-purple-900/30 border border-blue-500/20 rounded-2xl p-5 shadow-lg relative">
                                ...
                                <div className="flex items-end gap-2 mb-2">
                                    <span className="text-5xl font-bold text-white tracking-tighter tabular-nums drop-shadow-glow">{days}</span>
            ...
            <EditTargetCardModal isOpen={!!editTargetCard} onClose={() => setEditTargetCard(null)} cardMeta={editTargetCard} />
            <ConfirmModal
